    ...
    <div
      :class="{
        'el-tiptap-editor__menu-bubble--active':
          ...
          && bubbleMenuEnable(generateCommandButtonComponentSpecs(editorContext))
      }"
      :style="`
...
<script lang="ts">
  ...
  export default class MenuBubble extends Vue {
    ...
    private bubbleMenuEnable(commandButtons): boolean {
      return (this.linkMenuEnable || this.textMenuEnable)
        && this.activeMenu !== MenuType.NONE
        && !(
          this.activeMenu === MenuType.DEFAULT
    ...
    $_getCurrentMenuType()
      :
      MenuType {
      if (isTableActive(this.editor.state)) return MenuType.TABLE;
